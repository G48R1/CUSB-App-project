...
import { INDICE_RACCOLTE_DIR, RACCOLTA_TEMPLATE_DIR } from "../../App/config/config.js";
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch(joinPath(INDICE_RACCOLTE_DIR, './indice-eventi.json'))
    ...
    .then(data => {
      ...
      data.forEach(canto => {
        ...
        iframe.src = joinPath(RACCOLTA_TEMPLATE_DIR, "evento.html")+`?path=${encodeURIComponent(path)}&title=${encodeURIComponent(canto.name)}&numero=${canto.numero}`+(canto.tonality ? `&tonality=${encodeURIComponent(canto.tonality)}` : "");
